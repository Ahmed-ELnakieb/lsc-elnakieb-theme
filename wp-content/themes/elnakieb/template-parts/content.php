...
<article id="post-<?php the_ID(); ?>" <?php post_class('egx-blog-7-card'); ?>>
	...
	<div class="card-content">
		<div class="meta-data">
			<span class=" name"><i class="fa-solid fa-circle-user"></i> <?php esc_html_e('By', 'elnakieb');?> <?php the_author();?></span>
			<span class=" name"><i class="fa-regular fa-comment-dots"></i> (<?php echo esc_attr(get_comments_number());?>) <?php esc_html_e( 'comments', 'elnakieb' )?> </span>
		...
		<a class="blog-list-btn" href="<?php the_permalink();?>">
			<span class="btn-text"><?php if(!empty($blog_btn_text)){echo esc_html($blog_btn_text);}else{esc_html_e( 'Read More', 'elnakieb' );}?></span>
			<span class="btn-icon">
